...
import { getSubCategoriesForMain, MAIN_CATEGORIES } from "@/lib/catalog";
...
function CategoryInlineNav({
  ...
  return (
    <nav aria-label="Store navigation" className="flex max-w-full flex-wrap items-center justify-center gap-1 sm:gap-1.5">
      {MAIN_CATEGORIES.map((main) => {
        const subCategories = getSubCategoriesForMain(main.slug);
 ...
 * `?q=` is applied in an effect so server HTML always matches the first client paint.
 */
function HeaderSearchFormView({ inputRef }: { inputRef?: RefObject<HTMLInputElement | null> }) {
  return (
    <form action="/" method="get" className="flex max-w-full items-center gap-2">
      <input
        ref={inputRef}
        type="text"
        name="q"
...
function HeaderSearchFormInner() {
  ...
  return <HeaderSearchFormView inputRef={inputRef} />;
}
      ...
      >
        ...
        >
          ...
          <div className="hidden min-w-0 flex-1 justify-center px-2 lg:flex">
            <CategoryInlineNav pathname={pathname} productSidebarNav={productSidebarNav} />
          </div>
...
/**
 * TopNav is not wrapped in `<Suspense>` here: a shell fallback (`<section>` only) vs `TopNavInner`
 * (`<section><nav>…</nav></section>`) can produce different DOM on the server vs the first client pass
 * and trigger hydration attribute mismatches. `HeaderSearchForm` keeps its own Suspense for
 * `useSearchParams()`.
 */
