...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY?.trim()) {
      console.error(
        "[tax-invoice] SUPABASE_SERVICE_ROLE_KEY is not set; store_orders is not readable under RLS with the anon key.",
      );
    ...
    /** Signed-in customer using My account `orderId=` link (not admin, not tracking token). */
    let blockTaxInvoiceWithoutReference = false;
    ...
    if (token) {
      ...
      if (adminOk) {
        ...
        blockTaxInvoiceWithoutReference = true;
    ...
    if (orderErr) {
      ...
      return NextResponse.json({ error: "Could not load this order from the database." }, { status: 503 });
    }
    if (!order) {
    ...
    let invoiceRefColumnReadable = false;
    const refRes = await supabase.from("store_orders").select("invoice_reference").eq("id", order.id).maybeSingle();
    if (
      !refRes.error &&
      refRes.data &&
      typeof (refRes.data as { invoice_reference?: unknown }).invoice_reference !== "undefined"
    ) {
      invoiceRefColumnReadable = true;
      const v = (refRes.data as { invoice_reference: string | null }).invoice_reference;
      invoice_reference = v == null || typeof v === "string" ? v : String(v).trim() || null;
    }

    if (
      blockTaxInvoiceWithoutReference &&
      invoiceRefColumnReadable &&
      !(invoice_reference ?? "").trim()
    ) {
      return NextResponse.json(
        {
          error:
            "Invoice download will be available once your invoice number has been added to this order (same as admin Customer Invoices).",
        },
        { status: 403 },
      );
    }
    ...
    if (linesErr) {
      console.error("[tax-invoice] store_order_items:", linesErr.code, linesErr.message);
      return NextResponse.json({ error: "Could not load order lines." }, { status: 503 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Could not generate invoice PDF." }, { status: 503 });
    }
    const filename = taxInvoiceFilename(orderForPdf.order_number);
    const inline =
      request.nextUrl.searchParams.get("inline") === "1" ||
      request.nextUrl.searchParams.get("inline")?.toLowerCase() === "true";
    ...
    console.error("[tax-invoice] unexpected:", err);
    return NextResponse.json({ error: "Invoice download temporarily unavailable." }, { status: 503 });
  }
}
