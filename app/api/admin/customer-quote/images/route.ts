...
export async function POST(req: Request) {
  ...
  const quoteRequestId = String(formData.get("quote_request_id") ?? "").trim();
  const folder = quoteRequestId && /^[0-9a-f-]{36}$/i.test(quoteRequestId)
    ? `customer-quote/${quoteRequestId}`
...
export async function DELETE(req: Request) {
  ...
  if (!storagePath.startsWith("customer-quote/")) {
