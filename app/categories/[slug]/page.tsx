...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const totalPages = Math.max(1, Math.ceil(brandFiltered.length / CATEGORY_BROWSE_PAGE_SIZE));
  const currentPage = Math.min(requestedPage, totalPages);
  const offset = (currentPage - 1) * CATEGORY_BROWSE_PAGE_SIZE;
  const pageItems = brandFiltered.slice(offset, offset + CATEGORY_BROWSE_PAGE_SIZE);
    ...
    <main className="min-h-screen bg-white pt-[var(--site-header-height)] text-brand-navy">
      ...
      <MainWithSupplierRail>
        ...
        <section className={`${SITE_PAGE_ROW_CLASS} py-10`}>
          ...
          {totalPages > 1 && brandFiltered.length > 0 ? (
            <nav
            ...
            >
              {currentPage > 1 ? (
                <Link
                  href={pageHref(slug, currentPage - 1, brandParamEffective)}
              ...
              <p className="text-brand-navy/70">
                ...
                <span className="text-brand-navy/55"> ({brandFiltered.length} products)</span>
              </p>
