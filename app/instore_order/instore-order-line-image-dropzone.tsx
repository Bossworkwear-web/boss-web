...
import { storagePathFromProductionAssetsUrl } from "@/lib/store-order-reference-image-urls";
...
export function InstoreOrderLineImageDropzone({ imageUrls, onImageUrlsChange }: Props) {
    ...
    async (files: FileList | File[]) => {
      ...
      try {
        for (const file of list) {
          ...
          const res = await fetch("/api/instore-order/images", {
            method: "POST",
            body: fd,
          });
  ...
  async function removeImage(url: string) {
    const storagePath = storagePathFromProductionAssetsUrl(url);
    if (storagePath) {
      try {
        await fetch("/api/instore-order/images", {
          method: "DELETE",
    ...
    <div className="space-y-3">
      ...
      {imageUrls.length > 0 ? (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {imageUrls.map((url) => (
            <li key={url} className="relative overflow-hidden rounded-lg border border-brand-navy/15 bg-white shadow-sm">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={url} alt="" className="aspect-square w-full bg-brand-surface/40 object-contain p-1" />
              <button
                type="button"
        ...
        <p className="text-xs text-brand-navy/55 print:hidden">No images attached yet.</p>
      )}
    </div>
  );
}
