...
import { isInstoreOrderLocalDevActionBypass } from "@/lib/instore-order-access";
import { INSTORE_WALK_IN_PICKUP_ADDRESS } from "@/lib/instore-walk-in-constants";
...
function parseWalkInLines(formData: FormData): WalkInLine[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const notes = normalizeText(formData.get(`line_${i}_notes`));
    const hasContent = description || serviceType || color || size || notes || unitPriceCents > 0;
    if (!hasContent) continue;
    if (!description) {
      throw new Error(`Line ${i + 1}: enter a garment / item description.`);
    }
    lines.push({ description, serviceType, color, size, quantity, unitPriceCents, notes });
  }
  return lines;
}

export async function createInstoreWalkInOrder(formData: FormData): Promise<void> {
  if (!isInstoreOrderLocalDevActionBypass()) {
    try {
  ...
  const customerEmailRaw = normalizeText(formData.get("customer_email"));
  ...
  if (!customerEmailRaw && !customerPhone) {
  ...
  const customerEmail = resolveCustomerEmail(customerEmailRaw, customerPhone);
